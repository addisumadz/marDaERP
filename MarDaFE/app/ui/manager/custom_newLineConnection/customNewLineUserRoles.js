/**
 * Concrete User Role Verification Utility
 * Referencing UserAccount.java (role_id / userRole) and UserAccountRole.java (user_role_id / junction roles)
 */
...
// ─── Concrete Role Predicates (from UserRole & UserAccountRole) ───────────────
...
export function canRegisterApplication(roles) {
...
export function canAssignInstallationPlumber(roles) {
  return isTechnicalRole(roles);
}

export function canCompleteInstallation(roles) {
  return isTechnicalRole(roles);
}

export function canFinalizeCustomerActivation(roles) {
  return isCustomerServiceRole(roles);
}
