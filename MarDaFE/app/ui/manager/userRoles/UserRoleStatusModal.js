...
const UserRoleStatusModal = ({ open, onClose, onConfirm, mode, roleId }) => {
  ...
  const getTitle = () => {
    if (isDeactivateMode) return "Deactivate Role";
    if (isActivateMode) return "Activate Role";
    return "Confirm Action";
  };

  const getMessage = () => {
    if (isDeactivateMode) {
      return "Are you sure you want to deactivate this role? This will mark it as deleted.";
    }
    if (isActivateMode) {
      return "Are you sure you want to activate this role? This will mark it as active.";
    }
