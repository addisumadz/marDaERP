...
@Configuration
@EnableMethodSecurity // or @EnableGlobalMethodSecurity(prePostEnabled = true) for older Spring versions
    ...
    public CorsFilter corsFilter() {
        ...
        if (allowedOriginProps != null && !allowedOriginProps.isBlank()) {
            ...
            origins = List.of(
                ...
                "http://172.10.10.64:3000",
                "http://196.190.220.35:3000"
            );
        }
...
//                                // Add any other public Actuator endpoints if needed, e.g., "/actuator/health"
//                        ).permitAll()
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        // 🔒 Require authentication specifically for cashier payments
                        .requestMatchers(HttpMethod.PUT, "/api/card_managenment/*/cashier-payment").authenticated()
                        // ✅ Keep other card management endpoints open as before
