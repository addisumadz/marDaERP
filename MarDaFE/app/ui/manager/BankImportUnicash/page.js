...
const BankImportUnicash = () => {
  ...
  const { data: allBanks = [] } = useQuery({
    queryKey: ["billing-banks-all-for-unicash"],
  ...
  const handleFetchUnicash = async () => {
    if (!fromDate || !toDate) {
    ...
    try {
      setProcessing(true);
      const res = await bankUnicashService.fetchByDateRange(fromDate, toDate);
      if (res?.success) {
        toast.success("Fetched CSV successfully from Unicash API");
        setCsvFetched(true);
  ...
  const handleUploadCsv = async () => {
    ...
    try {
      setProcessing(true);
      const res = await bankUnicashService.uploadCsv(uploadFile, fromDate, toDate);
      if (res?.success) {
        setCsvFetched(true);
  ...
  const handleProcessUnicash = async () => {
    if (!fromDate || !toDate) {
      toast.error("Please select both from and to dates");
      return;
    }
    ...
    try {
      ...
      const start = await bankUnicashService.startProcessAsync(fromDate, toDate, kifyaWer);
      ...
      progressTimerRef.current = setInterval(async () => {
        try {
          const p = await bankUnicashService.getProgress(jid);
          setProgress(p);
          ...
          if (p?.done) {
            ...
            if (logsTimerRef.current) clearInterval(logsTimerRef.current);
            try {
              const res = await bankUnicashService.getProcessResult(jid);
              if (res?.success) {
  ...
  const handleApplyUnicashPayments = async () => {
    try {
      ...
      const updates = processResult.newPayments.map((p) => ({
        ...
        updates: {
          ...
          uBankPaidConfirmationCode: p.bankPaidConfirmationCode,
          uBankPaidAgentId: p.bankPaidAgentId,
        },
      }));
      ...
      for (let i = 0; i < updates.length; i += batchSize) {
        ...
        try {
          const res = await unicashPaymentImportService.updateUnicashPayments(batch);
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Breadcrumb pageName="Bank Import - Unicash" />
      </Grid>

      <Grid item xs={12}>
        <Paper elevation={3} sx={{ padding: 3 }}>
          <Typography variant="h5" gutterBottom>
            Unicash Bank Payment Import
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
            Fetch payment data from Unicash API or upload a CSV file, process it, and apply new payments to bills.
          </Typography>
          ...
          {processResult && (
            <Box sx={{ mb: 3, p: 2, bgcolor: "grey.50", borderRadius: 1 }}>
              ...
              <Grid container spacing={2}>
                <Grid item xs={3}>
                  <Chip
                    label={`New Payments: ${processResult.newPayments?.length || 0}`}
                    color="success"
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={3}>
                  <Chip
                    label={`Already Paid: ${processResult.alreadyPaid?.length || 0}`}
                    color="info"
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={3}>
                  <Chip
                    label={`Not Found: ${processResult.notFound?.length || 0}`}
                    color="warning"
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs={3}>
                  <Chip
                    label={`Duplicates: ${processResult.duplicates?.length || 0}`}
                    color="error"
                    variant="outlined"
                  />
                </Grid>
                <Grid item xs="auto">
                  <Chip
                    label={`Skipped (Bank Not Found): ${processResult.skippedBills?.length || 0}`}
                    color="warning"
                    variant="filled"
                  />
                </Grid>
            ...
            <Box sx={{ mb: 3, p: 2, border: "1px solid #4caf50", borderRadius: 1, bgcolor: "#f1f8e9" }}>
              ...
              <Box sx={{ display: "flex", gap: 2, alignItems: "center" }}>
                <Button
                  ...
                  onClick={handleApplyUnicashPayments}
                  sx={{ minWidth: 150 }}
                ...
                <Typography variant="body2" color="text.secondary">
                  This will update the bills to mark them as paid through Unicash
                </Typography>
              </Box>
              {applying && (
                <Box sx={{ mt: 2 }}>
...
export default BankImportUnicashWithProvider;
