...
// Request interceptor for dynamic URL and logging
apiClient.interceptors.request.use(
    async (config) => {
        ...
        try {
            ...
            if (customAddress) {
                // Construct new base URL: http://{address}/mardamobileapp/billing
                // Ensure no double slashes if user entered trailing slash, though unlikely with IP:Port
                const cleanAddress = customAddress.replace(/\/$/, '');
                // Handle if user typed http:// already or just IP
