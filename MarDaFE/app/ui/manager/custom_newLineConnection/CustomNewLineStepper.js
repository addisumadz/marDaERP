...
export const STAGES = [
  ...
  { key: "INSTALLATION_IN_PROGRESS", label: "ዝርጋታ ላይ", labelEn: "Connecting", dept: "Technical", order: 6 },
  { key: "INSTALLATION_COMPLETED", label: "ዝርጋታ ተጠናቋል", labelEn: "Installed", dept: "Technical", order: 7 },
  { key: "FINAL_ACTIVATION_COMPLETED", label: "ደንበኛው ነቅቷል", labelEn: "Active Customer", dept: "Customer Service", order: 8 },
];
...
export function getStatusBadge(status) {
  switch (status) {
    ...
    case "MATERIALS_COLLECTED":
      return { text: "ዕቃ ተወስዷል / ዝርጋታ ዝግጁ", color: "bg-teal-100 text-teal-800 dark:bg-teal-900/30 dark:text-teal-300" };
    case "INSTALLATION_IN_PROGRESS":
      return { text: "የመስመር ዝርጋታ ላይ", color: "bg-cyan-100 text-cyan-800 dark:bg-cyan-900/30 dark:text-cyan-300" };
...
export default function CustomNewLineStepper({ currentStatus }) {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 px-4 py-3 overflow-x-auto">
      <div className="flex items-center justify-between min-w-[760px] py-1">
        {STAGES.map((step, idx) => {
