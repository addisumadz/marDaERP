...
export default function InvItemsPage() {
  ...
  const [form, setForm] = useState({
    ...
    defaultUnitCost: "", isActive: true, isWaterMeter: false,
  });
  ...
  const resetForm = () => {
    setForm({ itemName: "", itemNameAm: "", description: "", categoryId: "", itemGroupId: "", unitOfMeasureId: "", reorderLevel: 10, reorderQuantity: 50, itemUsage: "BOTH", defaultUnitCost: "", isActive: true, isWaterMeter: false });
  ...
  const openEdit = (item) => {
    setForm({
      ...
      isWaterMeter: Boolean(item.isWaterMeter),
    });
                ...
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                  ...
                  <td className="px-5 py-3">
                    <div className="font-medium text-gray-900 dark:text-white flex items-center gap-1.5 flex-wrap">
                      ...
                      {Boolean(item.isWaterMeter) && (
                        <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300 border border-blue-200 dark:border-blue-800">
                          የውሃ ቆጣሪ
                        </span>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              {/* Water Meter Flag */}
              <div className="flex items-start gap-2.5 p-3 bg-blue-50/80 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/50 rounded-xl">
                <input
                  type="checkbox"
                  id="isWaterMeter"
                  checked={form.isWaterMeter}
                  onChange={(e) => setForm({ ...form, isWaterMeter: e.target.checked })}
                  className="w-4 h-4 mt-0.5 text-blue-600 rounded border-gray-300 focus:ring-blue-500 cursor-pointer"
                />
                <label htmlFor="isWaterMeter" className="text-xs font-bold text-blue-950 dark:text-blue-200 cursor-pointer">
                  የውሃ ቆጣሪ ዕቃ (Water Meter Item)
                  <span className="block text-[11px] font-normal text-blue-700 dark:text-blue-300 mt-0.5">
                    በአዲስ መስመር ዝርጋታ ከ 25% ትራንስፖርት፣ በጥገና ደግሞ ከ 25% እና 55% ክፍያዎች ነፃ የሚደረግ ልዩ ዕቃ
                  </span>
                </label>
              </div>
            </div>
