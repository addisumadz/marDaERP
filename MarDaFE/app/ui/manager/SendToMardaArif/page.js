...
const normalizeMardaArifMobile = (rawPhone) => {
...
const SendToMardaArifInner = () => {
  ...
  const { data: readings = [], isFetching, refetch } = useQuery({
    queryKey: ["mardaarif-send-readings", selectedKifyaWerMonth, selectedKifyaWerYear],
    queryFn: async () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (sendToBankFilter === "sent") {
      return base.filter(
        (r) => r.isSendToBankMardaArif === true || r.sendToBankMardaArif === true
      );
    }
    if (sendToBankFilter === "not_sent") {
      return base.filter(
        (r) =>
          r.isSendToBankMardaArif === false ||
          r.sendToBankMardaArif === false ||
          (r.isSendToBankMardaArif == null && r.sendToBankMardaArif == null)
      );
    }
    return base;
  }, [
  ...
  const table = useMaterialReactTable({
    columns: [
      ...
      {
        id: "isSendToBankMardaArif",
        header: "Sent to MardaArif",
        accessorFn: (row) =>
          (row.isSendToBankMardaArif === true || row.sendToBankMardaArif === true)
            ? "Yes"
            : "No",
      },
    ],
  ...
  const handleMarkAllSent = async () => {
    try {
      ...
      setProcessing(true);
      const res = await bankMardaArifService.markBillsSentToMardaArif({ readingIds });
      if (res && res.success) {
        toast.success(res.message || `Marked ${res.updatedCount || readingIds.length} bills as sent to MardaArif`);
        await refetch();
      } else if (res) {
  ...
  const handleSendCsv = async () => {
    try {
      ...
      const rows = csvEligibleData.map((row) => {
        ...
        const mobile = normalizeMardaArifMobile(row.customerPhoneNumber || row.phoneNumber);
      ...
      const file = new File([blob], "mardaarif_bill_data.csv", { type: "text/csv" });
      const res = await bankMardaArifService.forwardCustomersBillDataFile(file);
      if (res) {
        const message = typeof res.message === "string" && res.message.trim().length > 0
          ? `CSV sent to MardaArif: ${res.message}`
          : "CSV sent to MardaArif";
        ...
        try {
          const readingIds = csvEligibleData
            ...
            .filter((id) => id != null);
          if (readingIds.length) {
            await bankMardaArifService.markBillsSentToMardaArif({ readingIds });
          }
        } catch (markErr) {
  ...
  const handleDownloadCsv = () => {
    ...
    const headers = [
      "index", "bill_id", "bill_description", "bill_reason", "amount_due",
    ...
    link.href = url;
    link.download = "mardaarif_bill_data.csv";
  ...
  const handleSendSelected = async () => {
    try {
      ...
      for (const id of selectedIds) {
        ...
        const phoneNumber = normalizeMardaArifMobile(bill.customerPhoneNumber || bill.phoneNumber);
        ...
        const payload = {
          ...
          reason: billReason,
          validUntil: dueDate,
          email: "mardawbms@gmail.com",
          previousReading: bill.previousReading ?? 0,
          currentReading: bill.lastReading ?? 0,
          consumption: bill.consumption ?? 0,
        };
        console.log("MardaArif single-bill payload (client):", payload);
        try {
          await bankMardaArifService.submitSingleBill(payload);
          ok++;
      ...
      if (successIds.length) {
        try {
          await bankMardaArifService.markBillsSentToMardaArif({ readingIds: successIds });
        } catch (e) {
          toast.error(e?.response?.data?.message || e?.message || "Failed to mark some bills as sent to MardaArif");
        }
      }
      if (ok) toast.success(`Sent ${ok} bills to MardaArif`);
  ...
  const handleUpdateBankBills = async () => {
    try {
      ...
      if (!eligibleRows.length) {
        toast.error("Selected bills are not marked as sent to MardaArif");
        return;
      }
      ...
      const dueDateISO = formatDateISO(dueDateEC);
      const res = await bankMardaArifService.extendBillsWithPenalty({
        readingIds,
  ...
  const handleUpdateBulkCsv = async () => {
    try {
      ...
      const readingIds = eligibleRows.map((r) => Number(r.id));
      const dueDateISO = formatDateISO(dueDateEC);
      const updateRes = await bankMardaArifService.extendBillsWithPenaltyLocal({
        readingIds,
        extraPenalty: penalty,
        dueDate: dueDateISO,
      });
      ...
      const csvRows = eligibleRows.map((row) => {
        ...
        return [
          index, billId, billDesc, billReason, newTotal, customerId, fullName,
          dueDate, mobile, email, prevRead, currRead, consum,
        ];
      });

      const csvContent = [csvHeaders.join(","), ...csvRows.map((r) => r.map(escapeCSV).join(","))].join("\n");
      const blob = new Blob([csvContent], { type: "text/csv;charset=utf-8;" });
      const file = new File([blob], "mardaarif_bulk_update.csv", { type: "text/csv" });

      // 3. Upload CSV
      const uploadRes = await bankMardaArifService.forwardBulkUpdateCsvFile(file);
      if (uploadRes) {
        toast.success("Bulk update CSV sent to MardaArif!");
        await refetch();
      }
  ...
  const handleCancelBills = async () => {
    try {
      ...
      const readingIds = eligibleRows.map((r) => Number(r.id));
      const res = await bankMardaArifService.cancelBills(readingIds);
      if (res && res.success) {
  ...
  return (
    <Grid container spacing={3}>
      <Grid item xs={12}>
        <Breadcrumb pageName="Send Bills to MardaArif" />
      </Grid>
      <Grid item xs={12}>
        <Paper elevation={3} sx={{ p: 4 }}>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Typography variant="h5" sx={{ fontWeight: 'bold', color: 'primary.main' }}>
              MardaArif Bill Submission
            </Typography>
              ...
              <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2, height: '100%' }}>
                ...
                <Grid container spacing={2} alignItems="flex-end">
                  ...
                  <Grid item xs={12} lg={4}>
                    <Button variant="outlined" onClick={handleFilter} disabled={isFetching || processing} fullWidth sx={{ height: '40px' }}>
                  ...
                  <Grid item xs={4} lg={4}>
                    <TextField type="number" label="Penalty" InputLabelProps={{ shrink: true }} value={extraPenalty} onChange={(e) => setExtraPenalty(e.target.value)} size="small" fullWidth />
                  </Grid>
                </Grid>
              </Box>
            </Grid>
                ...
                <Grid container spacing={2}>
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={sendToBankFilter} label="Bank Sent" onChange={(e) => setSendToBankFilter(e.target.value)}>
                  ...
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={moneyCollectedFilter} label="Collected" onChange={(e) => setMoneyCollectedFilter(e.target.value)}>
                  ...
                  <Grid item xs={12} md={6}>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={selectedCustomerTypeId} label="Customer Type" onChange={(e) => setSelectedCustomerTypeId(e.target.value)} disabled={isCustomerTypesLoading}>
                        <MenuItem value=""><em>All Types</em></MenuItem>
                        {customerTypes?.map((type) => (
                          <MenuItem key={type.id} value={type.id}>{type.name || `Type ${type.id}`}</MenuItem>
                        ))}
                      </Select>
                    </FormControl>
                  </Grid>
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={selectedKebeleId} label="Kebele" onChange={(e) => { setSelectedKebeleId(e.target.value); setSelectedKetenaId(""); }} disabled={isKebelesLoading}>
                  ...
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth disabled={!selectedKebeleId}>
                      ...
                      <Select value={selectedKetenaId} label="Ketena" onChange={(e) => setSelectedKetenaId(e.target.value)} disabled={isKetenasLoading || !selectedKebeleId}>
                        <MenuItem value=""><em>All</em></MenuItem>
                        {ketenas?.map((ket) => (<MenuItem key={ket.id} value={ket.id}>{ket.name}</MenuItem>))}
                      </Select>
                    </FormControl>
                  </Grid>
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth>
                      ...
                      <Select value={selectedBranchId} label="Branch" onChange={(e) => { setSelectedBranchId(e.target.value); setSelectedReaderId(""); }} disabled={isBranchesLoading}>
                        <MenuItem value=""><em>All</em></MenuItem>
                  ...
                  <Grid item xs={6} md={3}>
                    <FormControl size="small" fullWidth disabled={!selectedBranchId}>
                      ...
                      <Select value={selectedReaderId} label="Reader" onChange={(e) => setSelectedReaderId(e.target.value)} disabled={isReadersLoading || !selectedBranchId}>
                        <MenuItem value=""><em>All</em></MenuItem>
                        {readers?.map((r) => (<MenuItem key={r.id} value={r.id}>{r.name}</MenuItem>))}
                      </Select>
                    </FormControl>
                  </Grid>
                </Grid>
            ...
            <Grid item xs={12}>
              <Box sx={{ border: '1px solid #e0e0e0', borderRadius: 2, p: 2, bgcolor: '#fafafa' }}>
                ...
                <Grid container spacing={2} alignItems="center">
                  <Grid item>
                    <Button variant="outlined" onClick={handleSelectAllFiltered} disabled={processing || !filteredData?.length}>
                      Select All
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="warning" onClick={handleSendCsv} disabled={processing || !filteredData?.length || !dueDate} sx={{ color: 'white' }}>
                      {processing ? <CircularProgress size={20} color="inherit" /> : "Send CSV"}
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="outlined" color="warning" onClick={handleDownloadCsv} disabled={processing || !csvEligibleData?.length || !dueDate}>
                      Download CSV
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="success" onClick={handleSendSelected} disabled={processing || !Object.keys(rowSelection).some((k) => rowSelection[k]) || !dueDate}>
                      {processing ? <CircularProgress size={20} color="inherit" /> : "Send Selected"}
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="secondary" onClick={handleUpdateBulkCsv} disabled={processing || !Object.keys(rowSelection).some((k) => rowSelection[k]) || !dueDate || !extraPenalty}>
                      {processing ? <CircularProgress size={20} color="inherit" /> : "Update Bulk CSV"}
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="primary" onClick={handleUpdateBankBills} disabled={processing || !Object.keys(rowSelection).some((k) => rowSelection[k]) || !dueDate || !extraPenalty}>
                      {processing ? <CircularProgress size={20} color="inherit" /> : "Update Penalty"}
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="error" onClick={handleCancelBills} disabled={processing || !Object.keys(rowSelection).some((k) => rowSelection[k])}>
                      {processing ? <CircularProgress size={20} color="inherit" /> : "Cancel Bill"}
                    </Button>
                  </Grid>
                  <Grid item>
                    <Button variant="contained" color="info" onClick={handleMarkAllSent} disabled={processing || !filteredData?.length}>
...
const SendToMardaArifPage = () => (
  <QueryClientProvider client={queryClient}>
    <SendToMardaArifInner />
  </QueryClientProvider>
);

export default SendToMardaArifPage;
