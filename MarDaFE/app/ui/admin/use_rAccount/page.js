...
const User = () => {
  ...
  // Function to open deactivation confirmation modal
  const openDeactivateConfirmModal = (user) => {
    setUserToDeactivate(user.original);
    setIsDeactivateConfirmOpen(true);
  };

  // Function to close deactivation confirmation modal
  const closeDeactivateConfirmModal = () => {
  ...
  const handleDeactivateUser = async () => {
    ...
    try {
      await userService.deactivateUser(userToDeactivate.id); // Call the service to deactivate the user
      fetchUsers(); // Refresh users after deactivation
      toast.success("User deactivated successfully!");
    } catch (error) {
      console.error("Error deactivating user:", error);
      toast.error("Failed to deactivate user.");
    } finally {
      closeDeactivateConfirmModal(); // Close the modal
    }
  };
    ...
    renderRowActions: ({ row }) => (
      <Box sx={{ display: "flex", gap: "1rem" }}>
        ...
        <Tooltip title="Change Password">
          <IconButton
            onClick={() => openChangePasswordModal(row.original)}
            sx={{ color: "purple" }}
          >
            <PasswordIcon />
          </IconButton>
        ...
        <Tooltip title="Deactivate">
          <IconButton
            color="error"
            onClick={() => openDeactivateConfirmModal(row)}
          >
            <DeleteIcon />
          </IconButton>
        </Tooltip>
    ...
    <>
        ...
        >
          ...
          <FormControl fullWidth required>
            ...
            {validationErrors.roles && (
              <span style={{ color: "red" }}>{validationErrors.roles}</span>
            )}
          </FormControl>
        </DialogContent>
        <DialogActions>
      ...
      >
        ...
        <DialogContent>
          ...
          <span style={{ color: "red" }}> "{userToDeactivate?.name}"</span>?
        </DialogContent>
        <DialogActions>
          <Button onClick={closeDeactivateConfirmModal}>Cancel</Button>
          <Button color="error" onClick={handleDeactivateUser}>
            Deactivate
          </Button>
        </DialogActions>
      </Dialog>
